...
#include "BRSGameSession.generated.h"


#define SETTING_SERVER_DISPLAYNAME FName(TEXT(""))

struct BRSENGINE_API FBRSGameSessionParams
{
 ...
 */
UCLASS(config=Game)
class BRSENGINE_API ABRSGameSession : public AGameSession
{
	GENERATED_BODY()
	
public:

	ABRSGameSession(const FObjectInitializer& ObjectInitializer);
...
protected:
	...
	FBRSGameSessionParams CurrentSessionParams;
	/** Current host settings */
	TSharedPtr<class FBRSOnlineSessionSettings> HostSettings;
	/** Current search settings */
	TSharedPtr<class FBRSOnlineSearchSettings> SearchSettings;
...
public:
	...
	*/
	bool HostSession(TSharedPtr< const FUniqueNetId> UserId, FName SessionName, const FString& GameType, const FString& MapName, bool bIsLAN, bool bIsPresence, int32 MaxNumPlayers, const FString& ServerDisplayName);
	...
	*/
	...
	FOnCreatePresenceSessionComplete& GetOnCreatePresenceSessionComplete() { return CreatePresenceSessionCompleteEvent; }

	/** @return the delegate fired when joining a session */
	FOnJoinSessionComplete& GetOnJoinSessionComplete() { return JoinSessionCompleteEvent; }

	/** @return the delegate fired when search of session completes */
	FOnFindSessionsComplete& GetOnFindSessionsComplete() { return FindSessionsCompleteEvent; }
